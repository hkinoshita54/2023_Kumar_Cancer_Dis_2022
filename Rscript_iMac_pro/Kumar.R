...
# seu_merge <- merge(seu_list[[1]], seu_list[2:40], add.cell.ids = c(1:20))
...
seu_int <- FindClusters(seu_int, resolution = 0.8)
...
seu_int <- readRDS(file = "RDSfiles2/seu_int.RDS")
...
FeaturePlot(seu_int,features = "S100A4", cols = c("lightgrey","darkred"), label = TRUE, repel = TRUE) + NoAxes() + NoLegend()

# subset epithelial, stromal and immune clusters
seu_epi <- subset(seu_int, idents = c(3, 5, 9, 12, 15))
seu_st <- subset(seu_int, idents = c(6, 7, 11, 16))
seu_imm <- subset(seu_int, idents = c(0:2, 4, 8, 10, 13, 14, 17))

saveRDS(seu_epi, file = "RDSfiles2/seu_epi2.RDS")
saveRDS(seu_st, file = "RDSfiles2/seu_st2.RDS")
saveRDS(seu_imm, file = "RDSfiles2/seu_imm2.RDS")
