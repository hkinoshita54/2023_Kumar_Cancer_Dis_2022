...
#   conda_env = "/usr/local/Caskroom/miniforge/base/envs/scvi-env", 
#   verbose = TRUE
# )
...
seu <- FindNeighbors(seu, reduction = "integrated.rpca", dims = 1:30, verbose = FALSE)
seu <- FindClusters(seu, resolution = 1, cluster.name = "rpca_clusters", verbose = FALSE)
seu <- RunUMAP(seu, reduction = "integrated.rpca", dims = 1:30, reduction.name = "umap.rpca", verbose = FALSE)
...
saveRDS(seu, file = "RDSfiles/seu_V5_lognorm_rpca_2.RDS")

# Check which cluster is epithelial, immune or stroma ----
seu <- readRDS(file = "RDSfiles/seu_V5_lognorm_rpca_2.RDS")
...
FeaturePlot(seu,features = "PLD1", cols = c("lightgrey","darkred"), label = TRUE, repel = TRUE) + NoAxes() + NoLegend()
...
seu_epi <- subset(seu, idents = c(5,9,11,13,15,25,30))
seu_str <- subset(seu, idents = c(8,12,14,19,20,26,27,29,32))
seu_imm <- subset(seu, idents = c(0:4,6,7,10,16:18,21:24,28,31,34))

saveRDS(seu_epi, file = "RDSfiles/seu_lognorm_rpca_epi.RDS")
saveRDS(seu_str, file = "RDSfiles/seu_lognorm_rpca_str.RDS")
saveRDS(seu_imm, file = "RDSfiles/seu_lognorm_rpca_imm.RDS")
